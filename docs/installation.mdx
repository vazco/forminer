...
To run the app with no issues currently, it's needed to update the `tsconfig.json` file and add:
...
Currently, we're not able to support `tsconfig.json` that includes `src` directory, for example:
...
We trying our best to resolve all remaining issues related to TypeScript validation.
