...
1.  Ensure you have the right versions of node and npm  
    Look at [Technical requirements (versions)](https://vazco1.atlassian.net/wiki/spaces/FB/pages/1277973).
...
You can install any theme, but remember to install it at `3.10.0` version.
...
If you will encounter problems running the app at this point please look at the 8. FormBuilder problems solving.
...
8\. FormBuilder problems solving

8.1 To run the app with no issues currently, it's needed to update the `tsconfig.json` file and add:
...
8.2 Issues with `tsconfig.json` that includes `src` directory, for example:
