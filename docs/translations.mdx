...
2. Translate them **in the form definition**. As the form definition is simply a JSON object (as described in [Form definition](form-definition)), you can modify it freely before passing it down to the `FormBuilder` component. That means you could translate all of the values up front and render a translated form. This requires some additional code but is seamless from the Forminer perspective.
